...
/**
 * Facilitates the purchasing of the technology cards in Ticket To Ride. Technologies change
 * the way that points are added to the players, the ways routes are purchased, the way locomotives are purchased, and
 * they also unlock routes that were previously unavailable to the player. To buy a technology, the player clicks on the 
 * Buy Technology button that is on the BottomPanel. The currency for technologies is paid in locomotive cards, or an 
 * appropriate amount of regular train cards that the player can use as well. When a technology is purchased, the 
 * cards used as currency are removed from the player's hand and in most cases, added to the discarded pile, Then, 
 * the player obtains the technology, it remains in their hand for the rest of the game unless otherwise specified. 
 * 
 */
public class BuyTechnologyPanel extends JPanel {
    ...
    /**
     *Overrides the paintComponent method of JPanel so in order to display and update the display of the technology cards as they are 
     *purchased.
     */
    @Override
    ...
    /**
     * Setter for the current player, since player is private in BuyTechnologyPanel.
     */
    ...
    private class Handler extends MouseAdapter {
        /**
         * Override of the mouseClickedMethod in MouseAdapter. Handles clicking on various objects on the current panel,
         * including the back arrow, any trainCard, and the confirmPurchase button. Also covers the logic of 
         * decrementing and incrementing respective quantities when trainCards and RightPanel.occurrences are clicked on.
         */
        @Override
        public void mouseClicked(MouseEvent e){
            if(e.getSource().equals(back_arrow)){
                //if clicking on the back arrow, escape this JPanel and return to the game board screen
                    ...
                    if(label.getIcon().equals(tech.getIcon())){
                        ...
                        for(int i = 0; i < tech.getCost(); i++){
                            ...
                            if(ConvertPanel.tradedLocomotives < RightPanel.occurrences[0])
                            {
                                ...
                                //in this instance of BuyTechnologyPanel
                                RightPanel.occurrences[0]--;
                            }
                            else
                            {
                                /**
                                 *handles locomotives that are created by purchase and not drawn
                                 *from the deck. These purchased locomotives cannot be discarded 
                                 *into the discardedTrainCards ArrayList because they technically
                                 *do not exist.
                                 */
